...
- [ ] Config/環境変数読込
  - [ ] pydantic/Envで設定を読み込む
  - [ ] 必須/任意のバリデーション（例: DROPBOX_ACCESS_TOKEN 必須 in MVP）
...
- [ ] StateStore（JSON版）
  ...
  - [ ] 読み書き・初期化
- [ ] SyncPlanner
  - [ ] Stateとの差分計算（新規/更新のみ抽出）
  - [ ] 命名規則（安全なファイル名への正規化の初版）
- [ ] Transfer（最小）
  - [ ] ダウンロード→一時フォルダ→Dropboxアップロード
  - [ ] 成功時にState更新
- [ ] CLI
  - [ ] `sync` サブコマンド
  - [ ] `--dry-run`（アップロードせず計画のみ表示）
  - [ ] ログ（INFO/ERROR）
